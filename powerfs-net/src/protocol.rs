...
pub enum MsgType {
    ...
    // ===== Cross-shard rename 2PC (client-coordinated, §4.1) =====
    // Each phase is routed by the client to the correct shard leader;
    // followers return STATUS_ERR_REDIRECT. No filer-to-filer forwarding.
    /// Phase 1 source: remove source dir entry + record rename intent.
    /// Request: RenameId + ParentIno + Name + Ino + NewParentIno + NewName
    RenamePrepareSource = 0x0044,
    /// Phase 1 dest: install target dir entry + record rename intent
    /// (captures replaced_inode when target already exists).
    /// Request: RenameId + ParentIno + Name + Ino + (no old path needed)
    RenamePrepareDest = 0x0045,
    /// Phase 2 source: drop source intent (entry already removed in prepare).
    /// Request: RenameId + ParentIno + Name
    RenameCommitSource = 0x0046,
    /// Phase 2 dest: drop dest intent + DecrementNlink(replaced_inode).
    /// Request: RenameId + ParentIno + Name
    RenameCommitDest = 0x0047,
    /// Rollback: undo a prepared side (restore dir entry + drop intent).
    /// Request: RenameId + RenameSide + ParentIno + Name + Ino
    RenameAbort = 0x0048,
    /// Update an inode record's name + parent_inode in-place after a
    /// cross-shard rename. Routed to calculate_shard(inode). Best-effort:
    /// failure does not affect dir-entry correctness (getattr falls back to
    /// the dir entry's name/parent).
    /// Request: Ino + NewName + NewParentIno
    RenameInode = 0x0049,
    /// Decrement an inode's nlink (and delete if it reaches zero). Routed to
    /// calculate_shard(inode). Used by the cross-shard rename coordinator to
    /// release a replaced target inode that may live on a different shard.
    /// Request: Ino
    DecrementNlink = 0x004A,
...
impl MsgType {
    pub fn from_u16(v: u16) -> Option<Self> {
        match v {
            ...
            0x0044 => Some(Self::RenamePrepareSource),
            0x0045 => Some(Self::RenamePrepareDest),
            0x0046 => Some(Self::RenameCommitSource),
            0x0047 => Some(Self::RenameCommitDest),
            0x0048 => Some(Self::RenameAbort),
            0x0049 => Some(Self::RenameInode),
            0x004A => Some(Self::DecrementNlink),
...
pub enum FieldId {
    ...
    AdminGrpcPort = 0xD7,
    /// Rename 2PC unique identifier (string). Generated by the client as
    /// "{client_id}:{seq}". Ties source/dest intents and makes every Raft
    /// command idempotent on retry.
    RenameId = 0xD8,
    /// Rename 2PC side selector (u8): 0 = source, 1 = dest. Used by
    /// RenameAbort to identify which side to roll back.
    RenameSide = 0xD9,
    /// Inode replaced by a cross-shard rename (u64). Recorded in the dest
