...
| Table 1 | 冗余轴分类框架 | 写作 |
| Table 2 | 冗余轴 × workload 边界表（roadmap 7.3：复制轴 1.0 / 相似轴 ≈0） | **R1 已完成** |
| 格式槽位 | safetensors 20 步同序列对照（64K+ 全 0） | **R2 已完成** |
...
1. ~~R1 冗余轴夹具 + 分析~~（2026-09-11 完成，roadmap 7.3：复制轴 1.0 / 相似轴 0）；
2. ~~R2 safetensors 槽位~~（2026-09-11 完成，同序列 64K+ 全 0）；
3. ~~数据回填 roadmap 7.3、Table 2 定稿~~；
4. 测量阶段收尾，下一步可选：读侧实验（留全文，不阻塞短文）或 2026-12 起按本 outline 成文，2027 春末前内部评审 → 投稿。
