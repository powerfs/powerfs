...
### 8.1 实现细化（P2 落地记录，与上文伪代码的偏离）

实施 T6 时发现「搬移以普通 DATA 记录写入」在两个组合场景下不成立，落地形态调整如下（字节格式见 §4.3 记录类型 `0x08 GC_MIGRATE` / `0x09 TOMB_PURGE`）：

1. **搬移记录独立为 GC_MIGRATE（0x08），条件应用**。搬移「读条目 → 重写」与在线写并发：普通 DATA 记录的 lsn 一定晚于用户并发覆写，会以旧数据凭更大 lsn 反超用户更新。GC_MIGRATE payload 自包含 `needle_id | expect_version | data_len | data`，仅当 needle 当前 `version_lsn == expect_version` 时覆写应用，否则跳过（用户胜出，记录留盘为无主副本随段回收）。
2. **重放条件：缺席即应用**。原段被 case A 删除后，needle 的 DATA 历史记录从盘面消失，GC_MIGRATE 可能是唯一可重放副本。统一应用规则（在线/重放同一函数）：purge 标记覆盖 → 压制；存在更新 tombstone → 跳过；条目版本匹配 → 覆写；条目版本不符 → 跳过；条目缺席且无 tombstone/purge → 建立条目。NeedleEntry 以 flags bit0 标记物理形态（GC_MIGRATE 帧 20B payload 头 vs DATA 帧 12B 头），读路径/二次搬移按标记分派解析与 CRC 种类。
3. **case C 必须持久化 TOMB_PURGE（0x09）**。仅在内存把 tombstone 迁入死副本账本存在复活漏洞：DELETE 记录所在段可能先于含更早 DATA/GC_MIGRATE 副本的段被 case A 回收，重放失去删除事实后孤儿副本让已删 needle 复活。purge 时先向活跃段 strict 写 `needle_id | delete_lsn` 标记（durable 后再回收任何段），标记经 checkpoint 持久化；重放时压制同 needle 一切 lsn ≤ purge_lsn 的记录。标记之后的新 DATA（删除后重新创建）正常建立并清除标记（新世代）。
4. **标记修剪有界**：段 lsn 区间单调，当现存最老段 `base_lsn > purge_lsn` 时物理上不再可能存在被压制的孤儿，GC 每轮清理该标记（checkpoint 随之收敛）。
5. 在途读安全当前由读路径 ENOENT/UnexpectedEof 重试一次（重查索引）覆盖；epoch guard（段级 refcount）留待后续增强。
...
1. 配置项为 `WalEngineConfig.recovery_mode: RecoveryMode`（`TolerateTail` 默认 / `PointInTime` / `Absolute`），重放器返回结构化 `StopPoint { seg_id, offset, reason }` 与 `abandoned` 段清单；截断/停点动作幂等。
2. 尾撕裂（仅最后一段）三档**都**物理截断到最后完整帧——未完成写入不属于校验失败；告警级别区分：TolerateTail = info、PointInTime = warn、Absolute = error（挂载仍成功）。
3. 中部断链（非尾段撕裂 / CRC 失败 / LSN 回退 / payload 语义损坏）：Absolute 拒载且**不截断**（盘面原样保留供审计）；其余两档截断停点段（Corrupt 截到最后完整帧末，LsnRegression/MalformedPayload 截到越界/损坏帧首），停点段物理封段（reopen+seal 幂等），后继段 unlink + fsync 目录 + manifest 移除，然后开新段续写（§10 步骤 5）。
4. checkpoint 锚点超前于恢复点时（ckpt 状态引用了将被弃置的段）：丢弃已装载 ckpt，全量重放重建索引，并物理删除 `applied_lsn > 恢复点 lsn` 的 ckpt 文件（`remove_future_ckpts`），superblock 回指剩余最新 ckpt；否则下次启动会装载超前状态。由于首轮（ckpt）重放已物理截断损坏段，全量重放以 `forced_stop` 停点参数在截断边界停扫，避免损坏证据消失后越过恢复点。
5. 三档动作矩阵（尾撕裂/中部断链 × 三模式）在 `replay.rs` 与 `engine.rs` 单测中逐项覆盖，含「停点 → 续写 → 再次重开」与「超前 ckpt 失效后多次重开」端到端用例。
...
| S5 | 完成 | `wal/engine.rs`：WalEngine 装配（flock 单写者锁 → SegManifest 扫描 → replay_all 重放 → 重开活跃段/以 last_lsn+1 开新段 → CommitQueue 以 last_lsn+1 起始 LSN 启动）；`SegSink` 桥接 CommitSink（Mutex\<SegWriter\> 串行化追加、克隆 fd 执行 sync 不阻塞追加、段满自动 seal→开新段→登记清单→fsync 目录项、记录超段容量拒绝）；write/read/delete/flush/flush_all/stats API（DATA/DELETE payload 编码、read 索引 O(1) 定位 + pread + 帧 CRC 重算校验、delete 前存在性检查、tombstone 7 天保留期）；nix Flock 守卫（替代已弃用 flock 调用，Drop 即释放，声明序保证排空先于解锁） | 50 个单测全绿（wal:: 全量）：write/read/delete/覆写 roundtrip 与垃圾记账、重开保持状态并接续 LSN/needle id、kill -9 式帧中崩溃恢复到 durable 前缀（strict 5 条可见 + async 窗口 5 条丢失 + 续写单调 + 二次重启稳定，I1/I2）、flock 拒绝第二个写者、小段容量多段换段 + 跨段重放恢复、flush barrier 提升 async durable、8 线程并发写全可见且统计一致、引擎存活期间位翻转读路径 CRC 检出、记录超段容量入队前拒绝；**修复**：open 新建段未登记 manifest 导致换段 seg_id 冲突（File exists）、在位损坏测试改为引擎存活期间注入（重开会在 replay 阶段拒绝，与设计一致） | e0f01cc9 |
| S6 | 完成 | `tests/wal_fault_sim.rs`：VOPR 式确定性故障模拟器——SplitMix64 种子生成随机操作序列（write 覆写/delete/flush，async:strict≈7:3，needle 池 1..=8 强制覆写/删除/复活交错）；执行日志记录每操作 (lsn, seg_id, 帧尾偏移) 与 durable 水位推进；**crash 注入**：逐操作边界 × 撕裂尾变体（0/7/16 字节非零撕裂）把盘面重建为「durable 前缀 + 撕裂尾」（删除后继段、截断覆盖段，等价 page cache 丢失 + 帧中断电）；**shadow model 断言**：恢复态 == 按 LSN 应用全部 durable 记录的终态（I2）、durable-ack 效果全可见（I1）、水位外 needle 全部 NotFound、统计与索引严格一致（I4 重放侧）、flushed_lsn == 水位（LSN 接续）、续写 strict + 二次重启稳定；40 种子 × 25 注入点 × 3 撕裂变体 = **3000 crash 场景**；小段 1KiB + 关预分配（工作负载内自然换段且免大文件 fallocate 开销）、逻辑 crash 免物理 fsync、种子并行回放（scoped threads，确定性按种子成立与调度无关） | 2 个测试全绿：crash 矩阵 3000 场景全部通过 I1/I2 断言（5.3s，8 并发）；种子回放确定性（seed=777 两次运行结果一致）；`cargo clippy --tests` 对 wal_fault_sim 无告警；wal:: 单测 50 个回归全绿 | 55446b92 |
| S7 | 完成 | **v1/v2 并行切换**（§13/§17 P1）：`config.rs` VolumeConfig 新增 `volume_engine`（needle\|wal，缺省 needle）与 `wal_segment_size`；`powerfs-core` 新增 `volume_engine.rs`（`EngineKind` + `VolumeEngine` enum 分发层，Box 包装消 clippy large_enum_variant，v1 Volume 与 v2 WalVolume 共享同一 API 面）与 `wal_volume.rs`（WalVolume 适配器：写=DATA 记录入组提交、blob 部分写=RMW 合并整条覆写 + off=0 整块快路径、flush_specific_needles=组提交持久化屏障、force_sync_on_write 映射 CommitMode::Strict、重放终态恢复 used/next_file_key、Full/Available 状态转换；worm_lock/restore 留位 P2、compact no-op 返回 (0,0)、NeedleInfo.checksum 映射帧 CRC32C u64 扩展）；`storage.rs` StorageManager enum 化（`new_for_engine` 构造、load_volumes 按目录形态 seg_*.log/volume.lock 识别 WAL 卷与 v1 卷实现混合并行、delete_volume 引擎分支清理）；`powerfs-volume/src/main.rs` 配置贯通 + **本地面 admin 子命令**（`powerfs-volume admin stats\|verify <volume_dir> --segment-size N`，不取锁不加载 config，§19.3 只读并发安全）；`wal/admin.rs` 只读检查（SegManifest 扫描 + 全量重放逐帧 CRC/哈希链校验 + 索引统计/撕裂尾报告）；`grpc_test.rs` 重构为宏参数化双引擎套件 | grpc_test 16/16 全绿（needle_engine + wal_engine 各 8 测试：create/delete/write/read/delete+ENOENT/blob 写读/meta）；wal::admin 单测全绿（inspect/verify 报告与重放终态逐字段一致）；CLI 端到端：真实 WAL 卷（5 write + 1 delete）admin stats 输出 volume_id/段清单/active=4/tombstone=1/last_lsn=6，admin verify 全量重放 OK；S6 故障模拟器 3000 crash 场景回归全绿；powerfs-core/volume/common 全量 311 通过（2 个失败为 write_coalescer/volume 既有失败，干净树复现确认与本阶段无关）；clippy 对新增代码无告警 | e5e31008 |
...
| T4 | 完成 | `wal/engine.rs`：checkpoint\_impl（冻结索引快照 → flush\_barrier(apply\_lsn) → 原子写 ckpt → CKPT\_ANCHOR strict 入组提交并 apply\_ckpt\_anchor 记入内存索引 → superblock 轮换）、CkptScheduler 后台线程（ckpt\_interval 定时 + ckpt\_lsn_distance 写入量触发，与写路径共享 EngineCore）；恢复路径（open：flock → superblock 装载 → ckpt 按 seq 逐级回退装载 → replay\_all 传入 ckpt 索引 + skip\_lsn=applied\_lsn，前缀帧 CRC/哈希链照常校验仅跳过 apply → 重开活跃段）；停机 ckpt\_on\_close（Drop 顺序：停调度 join → 停机 ckpt → 排空组提交 → 释放 flock）；`wal/replay.rs`：replay\_all 增加 index 初态 + skip\_lsn 前缀跳过（frames\_skipped 统计）；`wal/index.rs`：advance\_replay\_cursor（重放游标 ≥ applied\_lsn）；`wal/admin.rs` 只读检查保持全量重放（与运行中 server 并发安全） | 15 个 engine 单测全绿（ckpt 重开状态一致、续写恢复、损坏回退上一 ckpt、无 superblock 指针兜底重放、后台调度触发、停机 ckpt）；wal 全量 77 测试绿；故障模拟器 3000 场景回归绿（修复：fault sim 配置 ckpt\_on\_close=false 保持 crash=page cache 丢失语义） | 842c19b7 |
| T5 | 完成 | `wal/replay.rs`：`RecoveryMode{TolerateTail,PointInTime,Absolute}`（Default=TolerateTail）替换 tolerate_tail 布尔；`StopPoint{seg_id,offset,reason}` + `StopReason{TornNonTail,Corrupt,LsnRegression,MalformedPayload}`；`ReplayResult` 新增 stop/abandoned；扫描循环按档分派——尾撕裂三档均截断（级别由 engine 区分），中部断链 Absolute 原样拒载不截断、其余档物理截断停点段并弃后继段（Corrupt 截帧末、回退/坏 payload 截帧首）；`replay_with(forced_stop)` 供 ckpt 丢弃后全量重放在截断边界停扫。`wal/engine.rs`：config 增 recovery_mode；open 尾撕裂分级日志（info/warn/error）、中部停点 unlink 后继段 + fsync 目录 + reopen/seal 停点段 + manifest 收缩 + 封旧开新段；ckpt 锚点超前时全量重放 + `checkpoint::remove_future_ckpts` 物理失效 + superblock 回指 + next_ckpt_seq floor 防本进程序号复用。`wal/checkpoint.rs`：remove_future_ckpts。`wal/admin.rs`：只读检查显式 TolerateTail | replay 级：尾撕裂三档截断、torn_non_tail 矩阵（每档独立布局，Absolute 不截断校验）、lsn_regression 矩阵（回退截段首/弃置断言）；engine 级：中段损坏矩阵（Absolute 拒载 + 容忍档停点前缀可见/manifest seg1 sealed+新活跃段 base_lsn=3/续写/再次重开幂等）、超前 ckpt 失效（needle3-6 消失、ckpt 文件清空、三次重开终态稳定）；wal lib 93→97 测试绿（仅 2 个既有 v1 失败无关）；故障模拟器确定性崩溃矩阵 2 测试（3000+ 场景，1338s）回归绿（T5 基线）；T7 完成后最终回归 2 passed（1332.55s） | 2c945d9f（与 T7 合并提交：replay VolumeMeta 分派同期落地） |
| T6 | 完成 | `wal/gc.rs`（新增）+ engine/frame/index/replay/manifest/checkpoint 接线：GcWorker 后台线程（interval=None 关闭，手动 `gc()` 共用 run_gc_cycle，失败仅告警）；case C 过期 tombstone 读锁快照 → 逐条 purge_write strict 落 TOMB_PURGE(0x09) → apply_tomb_purge 迁 staging→garbage 并登记 purged 标记；候选扫描锁序 manifest→index，per_seg_bytes 分桶判定 case A（live=0 且 staging=0）/case B（dead/live>ratio 且 dead≥min_bytes，边界单测）；case B 存活 needle 快照 → 锁外 pread + 帧 CRC（flags 分派 DATA/GC_MIGRATE 帧形态，支持二次搬移）→ migrate_write 落 GC_MIGRATE(0x08) 条件应用 → flush_barrier 等 durable → 限速；case A unlink→fsync dir→manifest.remove + remove_seg_garbage 清账（修复 retain 后切片越界）；WalEngine workers Vec 统一管理 ckpt+GC 线程（Drop 停调度 join）；index：NEEDLE_FLAG_MIGRATED、apply_gc_migrate 统一规则（purge 压制/tombstone 否决/版本匹配覆写/缺席重建）、apply_data purge 压制与新世代清除、apply_tomb_purge、prune_purge_markers；checkpoint 新增 [purged] 区（header offset 116 计数 + 16B/条 needle_id|purge_lsn，启用 ver=1 保留字段），build/encode/decode/into_index 全链路；replay 分派 TombPurge；设计偏离（GC_MIGRATE 条件应用 + 持久化 purge 标记防复活 + 标记修剪）见 §8.1 | case A（搬移+整段删+重开缺席重建一致）、case B（搬移/阈值边界）、case C（purge 后 NotFound+staging 清零+重开一致）、保留期不 purge、限速 ≥49ms、GC 后 I4 自洽全绿；index 新增 6 个标记/条件应用单测；purge 标记经 ckpt 重开端到端测试；wal lib 90+ 测试绿；故障模拟器确定性崩溃矩阵 2 测试（3000+ 场景，1342s）回归绿 | c365cce2 |
| T7 | 完成 | `wal/frame.rs`：VolumeMetaPayload::volume_size/take_volume_size（mask 0x01，12B=mask u32+size u64，mask/缺字节按损坏）；`wal/index.rs`：WalIndex 增 volume_size（Default 0=未限定）+ getter/setter/apply_volume_meta（ShortPayload{op:"volume_meta[.size]"}，last_lsn 单调）；`wal/checkpoint.rs`：alloc_stats 48B→56B（尾部追加 volume_size，encode/decode/into_index/布局注释全链路）；`wal/replay.rs`：VolumeMeta 分支 apply_volume_meta（malformed→MalformedPayload 停点）；`wal/engine.rs`：EngineError 增 ResizeTooSmall{requested,required}/VolumeFull{volume_size,occupied,need}；resize_impl（shrink 前置 new_size≥used+staging+pinned，VOLUME_META strict 组提交→apply→立即轮换 superblock）；open 容量基线优先级 VOLUME_META 重放 > ckpt/superblock 装载值 > config.volume_size；写路径容量准入（occupied=used+staging+pinned，garbage 不占逻辑额度，cap=0 不限，物理超卖由段分配器兜底）；EngineStats 增 volume_size/free_bytes（未限定=u64::MAX）；WalEngine 增 resize/volume_size/free_bytes/is_full；store_superblock 从索引取容量并按 free≤0 写 VOLUME_STATE_FULL（Full 标记随 superblock 轮换点：resize/checkpoint/挂载）；`wal_volume.rs`：info.size 以引擎持久值为准、resize 映射（ResizeTooSmall→InvalidRequest、VolumeFull→OutOfSpace）、refresh_used 按 free 切 Full↔Available、free_space/准入同口径、VolumeFull 错误置 Full | frame payload roundtrip（12B/缺字节/mask 未置位/截断）、index apply（正常/旧 LSN 不回退游标/坏 payload 不改容量）、engine resize 全矩阵（grow/shrink roundtrip、缺口拒绝、写满 VolumeFull+sb.state、删除后 staging 仍 Full、gc purge 后空间恢复、grow 解除 Full+sb.state 回 0）、resize 持久化三路径（VOLUME_META 重放/ckpt alloc_stats/superblock-config 基线）；wal lib 97 测试绿、全 lib 150 绿（仅 2 个既有 v1 失败）；fmt/clippy 零警告；故障模拟器最终代码回归 2 passed（1332.55s）| 2c945d9f |
| T8 | 完成 | **本地面**：`wal/admin.rs` 增 run\_gc/run\_checkpoint/run\_resize（WalEngine::open 取 flock，AdminError::Busy 映射 EBUSY 文案引导走远程面/停服；retention 覆盖入 config）、list\_needles（TolerateTail 全量重放，--prefix/--limit，升序）、WalNeedleRow（seg/offset/lsn/crc）、inspect 报告增 pinned/volume\_size/free（0/unlimited 展示）；`wal_volume.rs` 增 wal\_gc/wal\_checkpoint/wal\_admin\_stats（WalAdminStats 13 字段）、compact 由 no-op 改为真 GC；`volume_engine.rs` 增 wal\_gc/wal\_checkpoint/wal\_resize/wal\_admin\_stats 分发（Needle→InvalidRequest/None）；powerfs-volume 离线子命令 list-needles/gc/checkpoint/resize。**远程面**：volume proto VolumeService 增 VolumeAdminStats/VolumeAdminGc/VolumeAdminCheckpoint/VolumeResize 四 RPC + 消息；server handler 全 spawn\_blocking 经 VolumeEngine 分发（v1 success=false+WAL-engine 文案）；master.proto MasterService 增四个 proxy RPC/消息（响应带 node=ip:port）；VolumeClientPool 增 admin\_stats/admin\_gc/admin\_checkpoint/resize\_volume（失败 invalidate 通道）；MasterGrpcServer::locate\_volume\_grpc（get\_volume\_info→get\_node\_info，卷未注册 NotFound/节点不可达 Unavailable）逐跳 info/warn 审计日志后转发；CLI `powerfs-cli volume status/stats/resize/gc trigger\|status/checkpoint trigger\|status`（§19.2，仅经 Master，-m/--volume，success=false 非零退出，字节 1024 进制人类可读） | admin 单测 3（含持锁三写命令全 Busy/只读 inspect 不受影响/释放后成功）；wal lib 99 绿；volume grpc 集成 19 绿（新增 3：WAL stats/checkpoint/gc/shrink 拒绝/grow 生效 e2e、v1 四 RPC 全拒、未知卷 not found 优雅失败）；master lib 99 绿、volume lib 18 绿；四 crate fmt/clippy 本面零警告（仅依赖 crate 既有警告） | 本地面 e190c6f5；远程面 a5196683 |
